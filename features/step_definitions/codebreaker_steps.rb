...
def message_should_include(message)
  messenger.string.split("\n").should include(message)
end

def fixnum_from(string)
  string.scan(/[[:digit:]]/).join.to_i
end
...
When /^I guess (.\s*.\s*.\s*.)$/ do |code|
  game.guess(code.split(/\s*/))
end
...
Then /^I should see "([^"]*)"$/ do |message|
  message_should_include(message)
end

Then /^the mark should be (.*)$/ do |mark|
  message_should_include(mark)
end

Then /^each color should appear between (\d+) and (\d+) times in each position$/ do |min, max|
  %w[r y g c b w].each do |color|
    (1..4).each do |position|
      count = @stats.count_for(color, position)
      count.should be_between(min.to_i, max.to_i), "expected #{count} to be between #{min} and #{max}"
    end
  end
end
